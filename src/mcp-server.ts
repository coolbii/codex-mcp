...
const OPENPENCIL_AUTHORING_GUIDANCE =
  ...
  '5) Every text node\'s fontFamily MUST be a font OpenPencil bundles: "Inter" (Latin UI) or "Noto Sans SC"/"Noto Sans TC" (Chinese). NEVER copy a reference\'s own font name (e.g. from extract_design_reference, like "F37 Analog"/"Graphik Web") into fontFamily — unbundled fonts render BLANK in the editor (a screenshot can still show them via system fallback, hiding it). Use the reference fonts only as inspiration for type behavior (editorial vs mono, scale, weight). Add a concrete fontWeight; size text to its box, wrap long headlines, prevent overlap. Put full-frame backgrounds as the LAST child of their parent (openpencil_move index 999). ' +
      ...
      async ({ url, viewportWidth, fullPage, timeoutMs }) =>
        invoke("extract_design_reference", {}, async () => {
          ...
          const summary =
            ...
            `Fonts (REFERENCE ONLY — do NOT set these as fontFamily; they are not bundled and render blank in OpenPencil. Use "Inter"/"Noto Sans SC" and only mimic the type behavior): ${r.fontFamilies.join(", ")} | type scale: ${r.typeScale.join("/")} | weights: ${r.fontWeights.join("/")}\n` +
